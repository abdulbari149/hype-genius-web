...
const CoversionRatePresets: React.FC<Props> = ({ defaultValue, setConversionRate }) => {
        ...
        <div className="max-w-[200px] w-full ml-3 relative">
          ...
          >
            <input
              {...getInputProps({
                className:
                  'text-[#27283080] bg-transparent flex-1 w-[100px]',
              })}
              disabled
              placeholder="Select"
            />
            <AiOutlineDown
              className={`text-[#27283080] ${isOpen ? 'rotate-180' : 'rotate-0'
                }`}
              size={16}
            />
          </div>
