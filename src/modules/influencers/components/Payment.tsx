...
const Payment = () => {
	...
	const data = useSelector((state: AppState) => state.influencers.influencer)
	...
	const onSubmit: FormikConfig<CreatePaymentData>['onSubmit'] = async (
		...
		try {
			const data = await paymentDataSchema.parseAsync({
				...
				channel_currency_id: values?.channel_currency_id
					? parseInt(values?.channel_currency_id.toString(), 10)
					: undefined,
			})
	...
	return (
		<InfluencerSidePanel>
			<Formik
				...
				onSubmit={onSubmit}
				validateSchema={toFormikValidationSchema(createPaymentSchema)}
			>
				{(formik) => (
					<div className="flex flex-col items-start w-full gap-6 px-[50px] mt-4 mb-[90px]">
						...
						<div className="">
							<p className="text-[20px] text-[#272830] font-[600]">
								{data?.influencer.firstName.charAt(0).toUpperCase() +
									data?.influencer.firstName.slice(1) +
									' ' +
						...
						<div className="flex flex-col gap-2">
							<p className="text-black text-[17px]">Current Contract</p>
							<div className="flex items-center gap-3">
								<div className="">
									...
									<span className="text-[15px] text-[#21A400] font-[500]">
										$
										{parseFloat(
											data.contract?.amount.toString(10) ?? '0',
										).toFixed(2)}
									</span>
									/
								...
								<div className="">
									<span className="text-[15px] text-[#21A400] font-[500] tracking-[1px]">
										{data?.contract?.uploadFrequency?.toString().toUpperCase()}
									</span>
									/
									<span className="text-[12px] font-normal text-[#000000]">
										mo
						...
						<div className="space-y-2">
							...
							<div className="bg-[#ECF0F4] max-w-[180px] rounded-xl flex items-center px-4 py-2 gap-3">
								...
								<input
									...
									value={formik?.values?.business_amount ?? undefined}
									onChange={(e) => {
							...
							<div className="flex gap-4">
								<CurrencySelector
									handleChange={(value, e) => {
										if (!e) {
											formik.setFieldValue('channel_currency_id', value)
											console.log(value)
										} else {
											formik.handleChange('channel_currency_id')(e)
										}
									}}
									value={formik.values.channel_currency_id ?? 0}
								...
								<div className="bg-[#ECF0F4] max-w-[180px] rounded-xl flex items-center px-4 py-2 gap-3">
									...
									<input
										...
										placeholder="enter here"
										name="channel_amount"
										value={formik?.values?.channel_amount ?? undefined}
										onChange={(e) => {
											const value = parseFloat(e.target.value)
											if (isNaN(value)) return
											formik.handleChange(e)
